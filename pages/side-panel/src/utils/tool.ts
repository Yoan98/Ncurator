...
import { STORAGE_DEPPSEEK_API_INFO, STORAGE_USER_PROVIDER_KEY_LIST_NAME } from '@src/utils/constant'
...
// 获取用户自定义的providerKey信息
export const getUserProviderKeyInfo = (providerId: string) => {
  let userProviderKeyListStr = localStorage.getItem(STORAGE_USER_PROVIDER_KEY_LIST_NAME)
  if (!userProviderKeyListStr) {
    return null
  }

  const userProviderKeyList: UserProviderKeyInfo[] = JSON.parse(userProviderKeyListStr)
  return userProviderKeyList.find(item => item.providerId === providerId) || null
}

// 保存用户自定义的providerKey信息
export const saveUserProviderKeyInfo = (userProviderKeyInfo: UserProviderKeyInfo) => {
  let userProviderKeyListStr = localStorage.getItem(STORAGE_USER_PROVIDER_KEY_LIST_NAME)
  if (!userProviderKeyListStr) {
    userProviderKeyListStr = '[]'
  }

  const userProviderKeyList: UserProviderKeyInfo[] = JSON.parse(userProviderKeyListStr)

  const existUserProviderKeyInfo = userProviderKeyList.find(item => item.providerId === userProviderKeyInfo.providerId)
  ...
  localStorage.setItem(STORAGE_USER_PROVIDER_KEY_LIST_NAME, JSON.stringify(userProviderKeyList))
}
// sleep函数
